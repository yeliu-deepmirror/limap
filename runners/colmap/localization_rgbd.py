...
def process_localization():
    ...
    line3d_ids = []
    line2ds_ref = []
    depth_ref_path = "/home/viki/Development/Alpha/Data/request/ref_depth.bin"
    image_ref_path = "/home/viki/Development/Alpha/Data/request/ref_image.jpg"
    pose_ref_path = "/home/viki/Development/Alpha/Data/request/ref_pose.txt"
    if line_map.model_loaded:
        image_query = cv2.imread(image_path)
        line3ds, line3d_ids, line2ds_raw, line2ds_ref, image_ref = line_map.locate_rgbd(image_query, depth_ref_path, image_ref_path, pose_ref_path, offset, False)
    ...
    if save_image:
        ...
        colors = matplotlib.cm.hsv(np.random.rand(len(line2ds))) * 255
        colors = colors.tolist()
        for i in range(len(line2ds)):
            line_2d = line2ds[i]
            cv2.line(image_query, transform_pixel(line_2d.start), transform_pixel(line_2d.end), colors[i], 3)
            line_2d_ref = line2ds_ref[i]
            cv2.line(image_ref, (int(line_2d_ref.start[0]), int(line_2d_ref.start[1])), (int(line_2d_ref.end[0]), int(line_2d_ref.end[1])), colors[i], 3)
            line_3d = line3ds[line3d_ids[i]]
            pixel_a = world_pt_to_qry_pixel(line_3d.start)
            pixel_b = world_pt_to_qry_pixel(line_3d.end)
            cv2.line(image_query, pixel_a, pixel_b, colors[i], 2)
        ...
        cv2.putText(image_show, "#line matches :" + str(len(line2ds_raw)),
                    (10, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (100, 255, 100), 1, cv2.LINE_AA)
        cv2.putText(image_show, "#point matches :" + str(len(point_2ds)),
                    (10, 60), cv2.FONT_HERSHEY_DUPLEX, 1, (100, 255, 100), 1, cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    ransac_cfg['ransac']['weight_line'] = 2.0
